 ...
 *
 * <ul>
 * <li><b>create</b>: Cria um novo produto, validando unicidade do nome e
 * existência das tags.</li>
 * <li><b>getById</b>: Busca produto por ID, podendo incluir
 * relacionamentos.</li>
 * <li><b>getAll</b>: Lista todos os produtos, com opção de incluir tags.</li>
 * <li><b>findRelatedProducts</b>: Busca produtos relacionados por tags
 * compartilhadas.</li>
 * <li><b>update</b>: Atualiza dados de um produto existente, validando nome e
 * tags.</li>
 * <li><b>delete</b>: Remove produto pelo ID.</li>
 * </ul>
 *
 * <p>
 * Utiliza {@link ProductRepository} e {@link TagRepository} para acesso aos
 * dados,
...
public class ProductService {
    ...
    public ProductResponseDTO create(ProductRequestDTO request, MultipartFile imageFile) {
        ...
        // Garantir que as tags associadas sejam carregadas antes de mapear para DTO.
        // Em alguns cenários a coleção pode não estar inicializada, então buscamos
        // as tags explicitamente e as definimos no produto salvo para que o
        // ProductMapper inclua no ProductResponseDTO.
        try {
            if (savedProduct.getId() != null) {
                savedProduct.setTags(new HashSet<>(tagRepository.findTagsByProductsId(savedProduct.getId())));
            }
        } catch (Exception e) {
            log.warn("Falha ao carregar tags para o produto recém-criado: {}", e.getMessage());
        }
     ...
     * Retorna uma página de produtos que compartilham tags com o produto
     * especificado.
     *
    ...
    private Set<Tag> validateTags(Set<Tag> tags) {
        // Aceita duas formas de entrada para tags:
        // - Tag com id preenchido: recupera do DB e valida existência
        // - Tag sem id (apenas name/description): busca por name e, se não existir,
        // cria uma nova tag
        ...
        for (Tag t : tags) {
            ...
            if (t.getId() != null) {
                // Tag referenciada por id: deve existir no banco
                Optional<Tag> existing = tagRepository.findById(t.getId());
                if (existing.isEmpty()) {
                    log.error("Tag não encontrada para id: {}", t.getId());
                    throw new TagNotFound("Tag não encontrada para id: " + t.getId());
                }
                resolved.add(existing.get());
            } else {
                // Tag sem id: deve ter nome válido
                if (t.getName() == null || t.getName().isBlank()) {
                    log.error("Tag sem id precisa ter um nome válido");
                    throw new IllegalArgumentException("Tag sem id precisa ter um nome válido");
                }

                // Tenta reutilizar tag existente por nome
                Optional<Tag> byName = tagRepository.findByName(t.getName());
                if (byName.isPresent()) {
                    resolved.add(byName.get());
                } else {
                    // Cria nova tag e persiste
                    Tag newTag = new Tag();
                    newTag.setName(t.getName());
                    newTag.setDescription(t.getDescription());
