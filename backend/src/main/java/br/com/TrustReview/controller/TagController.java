...
/**
 * Controller responsável pelo gerenciamento dos endpoints de
 * {@link br.com.TrustReview.model.Tag}.
 *
 * <p>
 * Disponibiliza operações REST para criação, consulta, atualização e remoção de
 * tags,
 * delegando a lógica de negócio ao serviço
 * {@link br.com.TrustReview.service.TagService}.
 ...
 * <li><b>getAll</b>: Lista todas as tags, podendo incluir produtos
 * associados.</li>
...
public class TagController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Tag criada com sucesso", content = @Content(schema = @Schema(implementation = TagResponseDTO.class))),
                        @ApiResponse(responseCode = "400", description = "Erro de validação", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "409", description = "Conflito - Tag com o mesmo nome já existe", content = @Content(schema = @Schema(implementation = ApiError.class))),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Tag encontrada com sucesso", content = @Content(schema = @Schema(implementation = ProductResponseDTO.class))),
                        @ApiResponse(responseCode = "404", description = "Tag não encontrada", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "500", description = "Erro interno do servidor", content = @Content(schema = @Schema(implementation = ApiError.class)))
        })
        @GetMapping("/{tag-id}")
         ...
         * Busca todas as tags.
         *
         * @param includeProducts Indica se os produtos associados às tags devem ser
         *                        incluídos na resposta
         * @return Lista de tags encontradas
         */
        @Operation(summary = "Busca todas as tags")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Tags encontradas com sucesso", content = @Content(schema = @Schema(implementation = TagResponseDTO.class))),
                        @ApiResponse(responseCode = "404", description = "Nenhuma tag encontrada", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "500", description = "Erro interno do servidor", content = @Content(schema = @Schema(implementation = ApiError.class)))
        })
        @GetMapping
        public ResponseEntity<List<TagResponseDTO>> getAll(
                        @RequestParam(name = "includeProducts", required = false, defaultValue = "false") boolean includeProducts) {
                return ResponseEntity.ok(service.getAll(includeProducts));
        }

        /**
         * Retorna as tags vinculadas a um produto pelo ID do produto.
         *
         * @param productId UUID do produto
         * @return Lista de tags associadas ao produto
         */
        @Operation(summary = "Busca tags por product id")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Tags encontradas com sucesso", content = @Content(schema = @Schema(implementation = TagResponseDTO.class))),
                        @ApiResponse(responseCode = "404", description = "Nenhuma tag encontrada para o produto", content = @Content(schema = @Schema(implementation = ApiError.class)))
        })
        @GetMapping({ "/product/{product-id}", "/products/{product-id}" })
        public ResponseEntity<List<TagResponseDTO>> getByProductId(@PathVariable("product-id") UUID productId) {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Tag atualizada com sucesso", content = @Content(schema = @Schema(implementation = ProductResponseDTO.class))),
                        @ApiResponse(responseCode = "400", description = "Erro de validação", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "404", description = "Tag não encontrada", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "409", description = "Conflito - Tag com o mesmo nome já existe", content = @Content(schema = @Schema(implementation = ApiError.class))),
                        @ApiResponse(responseCode = "500", description = "Erro interno do servidor", content = @Content(schema = @Schema(implementation = ApiError.class)))
